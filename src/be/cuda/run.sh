...
clang++ be_cuda_benchmark.cu -c  -std=c++11 --cuda-path=/usr/local/cuda-10.1 --cuda-gpu-arch=sm_61 -L/usr/local/cuda-10.1/lib64 -lcudart_static -ldl -lrt -pthread -save-temps -v


clang++ -emit-llvm main.cc --cuda-gpu-arch=sm_61 -O3 -c -pthread
clang++ -emit-llvm be_command_line_options.cc --cuda-gpu-arch=sm_61 -O3 -c -pthread
clang++ -emit-llvm be_benchmark.cc --cuda-gpu-arch=sm_61 -O3 -c -pthread


/data/jchen/open_source_template/build/compilation/kernelTranslator be_cuda_benchmark-cuda-nvptx64-nvidia-cuda-sm_61.bc kernel.bc 
/data/jchen/open_source_template/build/compilation/hostTranslator be_cuda_benchmark-host-x86_64-unknown-linux-gnu.bc host.bc
...
llc --relocation-model=pic --filetype=obj  be_command_line_options.bc
llc --relocation-model=pic --filetype=obj  be_benchmark.bc
